...
在Vue中，数组的响应式是通过特殊处理实现的，虽然无法实现直接通过下标修改元素实现响应式，但是，对于可以修改原数组的一些方法，Vue对这些方法通过特殊处理，使其达到响应式。
...
## 总结
可以看到，无论是处理数组还是暴露`$set`的API，核心的思想还是劫持操作+手动派发更新。这是目前`Object.defineProperty`的缺陷，期待ES6的`Proxy`能解决这些问题。